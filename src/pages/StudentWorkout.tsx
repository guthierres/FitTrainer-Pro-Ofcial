...
const StudentWorkout = () => {
  ...
  const loadStudentData = async () => {
    try {
      setIsLoading(true);
      console.log("Iniciando carregamento para studentNumber:", studentNumber);

      // 1. Busca o aluno para obter seu token.
      ...
      if (studentError || !studentData) {
        console.error("Aluno não encontrado ou inativo:", studentError?.message);
        toast({
          title: "Erro",
      ...
      console.log("Aluno encontrado. Definindo contexto de segurança...");
      setStudent(studentData);

      // 2. Define o contexto de segurança (RLS) com o token do aluno.
      // Esta chamada é crucial para que as próximas consultas funcionem.
      await setStudentContext(studentData.student_number, studentData.unique_link_token);

      // 3. Busca o plano de treino do aluno, agora com a RLS configurada.
      ...
      if (workoutError) {
        console.error("Erro na busca do plano de treino:", workoutError.message);
        toast({
      ...
      if (!workoutData) {
        console.warn("Nenhum plano de treino ativo encontrado para o aluno:", studentData.id);
        toast({
          title: "Aviso",
      ...
      // 4. Marca os exercícios concluídos para exibir o status.
      ...
      // Mapeia as sessões e exercícios para adicionar o status de 'isCompleted'.
      const sessionsWithCompletionStatus = workoutData.workout_sessions.map((session: any) => ({
        ...session,
        workout_exercises: session.workout_exercises.map((exercise: any) => ({
          ...exercise,
          isCompleted: completedExerciseIds.has(exercise.id),
        })),
      }));

      setWorkoutPlan({ ...workoutData, workout_sessions: sessionsWithCompletionStatus } as WorkoutPlan);

    } catch (error) {
      console.error("Erro inesperado ao carregar dados:", error);
      toast({
        title: "Erro",
        description: "Erro ao carregar dados do treino. Tente novamente.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const markExerciseAsCompleted = async (exerciseId: string) => {
    ...
    try {
      ...
      // Recarrega os dados para atualizar o estado da página
      loadStudentData();
    } catch (error) {
...
<head>
  ...
  <style>
    ...
    body { width: 80mm; margin: 0; padding: 3mm; font-family: 'Courier New', monospace; font-size: 9px; line-height: 1.3; color: #000; background: white; }
    .center { text-align: center; }
    .bold { font-weight: bold; }
    .separator { border-top: 1px dashed #000; margin: 3mm 0; width: 100%; }
    .small { font-size: 7px; }
    .exercise { margin: 2mm 0; padding: 1mm 0; border-bottom: 1px dotted #ccc; }
    .exercise-header { font-weight: bold; font-size: 10px; margin-bottom: 1mm; }
    .exercise-details { margin: 1mm 0; padding-left: 2mm; }
    .status-ok { font-weight: bold; }
    .status-pending { color: #666; }
    .header-box { border: 2px solid #000; padding: 2mm; margin-bottom: 3mm; }
    .footer-box { border: 1px solid #000; padding: 2mm; margin-top: 3mm; background: #f5f5f5; }
  </style>
</head>
<body>
    ...
    .map((exercise, index) => `
      <div class="exercise">
        ...
        <div class="exercise-details">
          ...
          ${exercise.reps_min && exercise.reps_max ? `<div>• Repetições: ${exercise.reps_min}-${exercise.reps_max}</div>` : exercise.reps_min ? `<div>• Repetições: ${exercise.reps_min}</div>` : ""}
          ...
          ${exercise.exercise.muscle_groups?.length > 0 ? `<div class="small">Músculos: ${exercise.exercise.muscle_groups.slice(0, 3).join(", ")}</div>` : ""}
          ${exercise.exercise.instructions ? `<div class="small">Execução: ${exercise.exercise.instructions.substring(0, 80)}...</div>` : ""}
        </div>
      </div>
    `).join("")}
  <div class="separator"></div>
